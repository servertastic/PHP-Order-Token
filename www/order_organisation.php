...
<body>
	...
	<div class="container order_organisation">
		<form id="validetta" action="<?php $_SERVER['PHP_SELF']; ?>" method="POST">
			...
			<?php else: ?>
				<?php // if org_name is now allowed then none of it will be required so redirect to review ?>
			...
			<?php if ($order->fieldIsAllowed('org_incorporating_agency')): ?>
				<div class="row">
					<div class="form-group col-md-6">
						<label for="org_incorporating_agency">Organisation Incorperating Agency</label>
						<input 
							type="text" 
							class="form-control" 
				...
				<div class="row">
					<div class="form-group col-md-6">
						...
						<?php foreach ($order->iso_codes as $value): ?>
							<option value="<?= $value['code'] ?>" <?= $order->isSelected('org_jurisdiction_country',$value['code']) ?>><?= $value['name'] ?></option>
						<?php endforeach ?>
						</select>
					</div>
				</div>
				...
				<div class="row">
					<div class="form-group col-md-6">
						...
						>
							...
							<?php foreach ($order->iso_codes as $value): ?>
								<option <?= $order->isSelected("org_address_country",$value['code']) ?> value="<?= $value['code'] ?>"><?= $value['name'] ?></option>
							<?php endforeach ?>
						</select>
					</div>
				</div>
